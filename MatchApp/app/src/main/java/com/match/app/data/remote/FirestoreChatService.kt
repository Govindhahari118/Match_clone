...
class FirestoreChatService @Inject constructor() {
    ...
    /** Send a message to Firestore. Includes Firebase UIDs for cloud-function push delivery. */
    suspend fun sendMessage(
        me: Long, peer: Long, body: String, 
        ...
        db.collection("chats").document(tid).collection("messages").add(data).await()
        // Update thread metadata for last message preview
        db.collection("chats").document(tid).set(
            hashMapOf(
                "lastMessage"  to body,
                "lastSentAt"   to System.currentTimeMillis(),
                "participants" to listOf(me, peer),
                "participantUids" to listOf(myFirebaseUid, peerFirebaseUid).filter { it.isNotBlank() }
            ),
    ...
    fun observeUnreadCount(userId: Long): Flow<Int> = callbackFlow {
        // Listen to all threads where this user is a participant
