...
suspend fun resolveProtectedMediaFile(context: Context, gsUrl: String): File? {
    ...
    return withContext(Dispatchers.IO) {
        ...
        try {
            // Re-authorize every resolve. The cached bytes are usable only after current rules allow
            // this signed-in viewer, so a later block/hide/privacy change invalidates old access.
